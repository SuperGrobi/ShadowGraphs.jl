"""
    ...
    project_local!(g::T, center_lon, center_lat) where {T<:AbstractMetaGraph}
...
and updates the `:crs` properity of `g` accordingly. All geometry is assumed to be in the `:crs` system.
"""
function CoolWalksUtils.project_local!(g::T, center_lon, center_lat) where {T<:AbstractMetaGraph}
